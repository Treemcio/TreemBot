module.exports = function(member){
  let lang = db.get(`${member.guild.id}.lang`)
   ...
   if(lang === null){
         db.set(`${member.guild.id}.lang`, "en")
    ...
    let papa = db.get(`${member.guild.id}.msgpa`)
    if(papa === undefined) papa = l.goodbay
...
ctx.font = '40px Comic Sans';
	ctx.fillStyle = '#000';
	ctx.fillText(`${papa}`, canvas.width / 2.5, canvas.height / 3.5);
...
request.get( url, function(err, resp, data) {
	...
	canvas.createPNGStream().pipe(fs.createWriteStream('goodbay.png'));
})
setTimeout(welcome, 1000)
function welcome(){
const attachment = new Discord.MessageAttachment('goodbay.png');
